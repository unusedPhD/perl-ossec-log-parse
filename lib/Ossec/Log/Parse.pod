...
=head1 SYNOPSIS
  ...
  while ( $events = $parse->getAlert() ) {
    print $events->{ts};
    # 1443175627.1028
    print $events->{ts.human};
    # 2015 Sep 25 06:07:07
    print $events->{type};     
    # mail 
    print $events->{group};    
    # syslog,fts,authentication_success
    print $events->{agent.name}; 
    # i7dev
    print $events->{agent.ip}; 
    # 10.0.0.4
    print $events->{location};
    # /var/log/auth.log
    print $events->{rule.id}; 
    # 10100
    print $events->{rule.level}; 
    # 4
    print $events->{rule.comment};
    # First time user logged in
    print $events->{full_log}; 
    # Src IP: 10.0.0.2
    # User: phirelight
    # Sep 25 06:07:06 i7dev sshd[17673]: Accepted publickey for phirelight from 10.0.0.2 port 44857 ssh2: RSA 00:00:00:00:00:00:00:00:00:00:00:00:00:00:00:00
  }
