...
=head1 SYNOPSIS
  ...
  while ( $alert = $parse->getAlert() ) {
    print $alert->{ts};
    # 1443175627.1028 (unix time stamp of when alert was recorded)
    print $alert->{ts.human};
    # 2015 Sep 25 06:07:07 (human readable time stamp of when alert was recorded)
    print $alert->{type};     
    # mail 
    print $alert->{group};    
    # syslog,fts,authentication_success
    print $alert->{agent.name}; 
    # i7dev (Either assigned agent name or local hostname)
    print $alert->{agent.ip}; 
    # 10.0.0.4 (Remote agent IP)
    print $alert->{location};
    # /var/log/auth.log (Log origin)
    print $alert->{rule.id}; 
    # 10100
